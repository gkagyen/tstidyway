...
This section is to build up on what we learnt in the previous section, where you have mastered date and time manipulations and feature engineering. You can now organise your time series data into a well-structured dataset with rich temporal features. But raw data, no matter how well prepared, rarely tells its story openly.
...
-   @sec-visualising-trends-with-ggplot2-and-autoplot: In this chapter, we will master the art of time series visualisation using both the versatile `ggplot2` package and the specialised `feasts::autoplot()` function. You will learn how to:
...
-   @sec-exploring-patterns-with-feasts-and-fabletools: Here we will dive deep into the `feasts` package, which provides specialised tools for time series exploration together with `fabletools`. You will discover how to
