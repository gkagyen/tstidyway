...
Heteroscedasticity refers to a change in the volatility and spread of a time series over time. Time series models (like ARIMA) assume that the variance of the forecast errors is constant. If the variance is changing it compromises the accuracy of confidence intervals and predictions.
...
While a formal statistical test for heteroscedasticity requires fitting a model and examining the squared residuals, the **Guerrero method** offers a quick quantitative diagnostic to determine if a transformation is required to achieve homoscedasticity or not. This is a technique to automatically select the optimal parameter ($\lambda$) for a Box-Cox transformation. The `feature()` function will help us do this.
...
The lambda ($\lambda$) value is interpreted based on its proximity to three key values: 1, 0, or a non-integer value. The lambda value produced for the sales series (0.73) is close to 1 which suggests no transformation. This strongly supports the initial visual inspection which showed that the monthly sales data is largely homoscedastic (stable variance).
...
Rather than guessing the optimal $\lambda$ value, we can use the `guerrero()` feature function as seen earlier from the `feasts` package to find it automatically.
...
The visual inspection shows a clear evidence of unequal variance, specifically increasing variance over time. The series is at a **low level** in the early periods (1960-1980 Q1) and the fluctuations are small relative to the mean (seasonal peaks and troughs closer together).

In the later period (1990-2008 Q4) the series is at a **high level** and the fluctuations are visually much larger (the distance between peaks and troughs has increased). This makes the series heteroscedastic because the **variance increase as the level (mean production) increases**.

This pattern is known as a multiplicative variance and it is a characteristic of a time series that exhibit growth. We will now calculate the optimal lambda needed for a Box-Cox transformation for this series.
...
The peaks and troughs for all quarters seem to have similar amplitudes throughout the years. Also notice how the y-axis scale changed from \~500 - 2500 in the original series to \~6.5 - 7.5 (log scale) in the transformed series giving it a much more constant variance over time. The transformed series is now suitable for ARIMA modelling as the constant amplitude of fluctuations satisfies the constant variance assumption.
...
|------------------------|------------------------|---------------------------------------|
| $\lambda=1$ | No Transformation $(Y_{t})$ | Series is already Homoscedastic |
...
The Left plot (Original Series) shows a clear linear relationship between its rolling mean and rolling standard deviation (SD). This is a clear presence of heteroscedasticity where variance grows with the series' level, which further confirms that the original series has a **variance proportional to the mean**. The nearly flat blue line in the left plot indicates how the transformation has now **stabilised the variance** across all levels. The rolling SD stays relatively constant (\~0.07 to \~0.125) regardless of the rolling mean.
...
We have tackled a fundamental challenge in time series forecasting; dealing with changing variances over time. We learnt that many forecasting models assume constant variance, but real world data often displays a changing variance, where variability decreases or increases with the series level.
