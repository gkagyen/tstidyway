...
The true power of the `fable` framework shines here - what traditionally required extensive statistical expertise and manual parameter tuning is now accessible through intuitive, automated functions that maintain the tidy workflow we have built throughout this book.
...
-   **Trend**: The long-term direction, This can be **N**one = no trend, **A**dditive = Linear trend or Damped (**Ad**) = A trend that levels off over time.
-   **Seasonality**: Regular repeating patterns over a fixed period. This can also be **N**one-for no seasonality, **A**dditive - for constant seasonal effect, or **M**ultiplicative - if seasonal effect scales with the level of the series.
...
The `ETS()` function fits an exponential smoothing model to the data. When no specific component configuration is provided, the function automatically searches for the optimal ETS model for our series. The best model is stored in the `auto` column of the **mable** output (`auto_ets`).We use the `report` function to display detailed summary of the chosen model, including its final configuration, estimated parameters, and information criteria.
...
Although the SES model only tracks the **level** of the series, it is essentially a more advanced version of the **MEAN** model, as it adapts its average based on the most recent data (via the smoothing parameter $\alpha$), rather than relying on a fixed historical average. This model would be useful for data like the **Ghana Annual GDP Growth Rate**, which fluctuates around a constant level ( with no clear trend or seasonality).
...
We will demonstrate this with the Annual Cereal Yield series from the `gh_ts` data, which clearly shows an upward trend over time. We first prepare the data by isolating the cereal yield indicator from the rest of the data and performing a train-test split before fitting the model.
...
Another key advantage of the **ETS framework** is the ability to extract and visualise the underlying components that drive the forecast, using the `component()` function which we saw in @sec-exploring-patterns-with-feasts-and-fabletools with the STL decomposition.
...
This code for the Holt-Winters model is explicitly configured to track all three components, Level (A), Trend (A) and Seasonality (A).
...
The forecasting operation for the Holt-Winters model takes the final components ($l_t$, $b_t$ and $s_t$) and projects them into the future $h$ steps. It is the sum of the last calculated level, trend and seasonal factor, following this equation:

Forecast Equation (Additive): $\hat{y}_{t+h|t}=l_t+hb_t+s_{t+h-m(k+1)}$

where $k$ = the integer or whole number part of $(h-1)/m$ (which ensures we use the most recent seasonal index for the forecast horizon). The `forecast()` function automatically calculates the this for us.
...
The resulting chart from plotting the original series (black line), the fitted values from the model (red line) and the 12 month forecast (blue line) together shows a powerful visual inspection for validating the models goodness of fit. The red line closely follows the original data, confirming that the smoothing parameters ($\alpha ,\beta ,\gamma$) were effectively optimised to track the historical level, trend and seasonality.

The projected blue line in the future exhibits a repeating seasonal pattern overlaid on a linear slightly upward trend, maintaining a constant magnitude for the seasonal peaks and troughs (this is a defining characteristic of the additive model). The prediction intervals (shaded area around forecast) widens as the forecast extends further into the future, reflecting the increasing uncertainty associated with predicting events further ahead in time.
...
The seasonal component has a clean, fixed amplitude waveform centred around zero, demonstrating regular quarterly pattern, which is extracted and projected into the forecast. This highly predictable and consistent pattern demonstrates strong quarterly seasonality in the cement production.
...
The Best fit model with lowest MASE is `hw_mult` (Holt-Winters Multiplicative). The MASE value confirms it has the lowest error compared to a **NAIVE** benchmark. The Damped Multiplicative and Additive models (`damped_mult`, `damped_add`) also show lower MASE error with the additive model showing a slightly higher error than the multiplicative.
...
-   **Seasonal Models** (`hw_mult`, `damped_mult`, `hw_add`, `damped_add`): These models project a cyclical pattern that matches the historical 12-month period. The two best-fitting models from the MASE comparison–Holt-Winters Multiplicative (`hw_mult`) and Damped Multiplicative (`damped_mult`), tracks the final period of the training data closely and projects the most reasonable and consistent seasonal cycle forward.
...
**Smoothing Parameters Tell a Story**: The $\alpha$, $\beta$, and $\gamma$ values reveal how stable or volatile your series is. High values suggest rapid changes, while low values indicate stable patterns.
