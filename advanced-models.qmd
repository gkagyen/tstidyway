# Advanced Forecasting Models: Exponential Smoothing (ETS) {#sec-advanced-forecasting}
...
We have seen the types of ETS models and how they operate now we will combine most of the possible configuration for fitting models and compare how they perform on the same data. We will use the monthly sales data
...
```{r}
glance(ets_comparison) |> arrange(AIC)

```

## Generating and Visualising Forecast from Multiple Models

We can generate and visualise the forecast to understand critically the goodness of fit for each model.
