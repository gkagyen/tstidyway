...
We fit multiple candidate models and evaluate their performance using statistical measures like AICc.The `glance()` function provides these statistical fit metrics. Lower $AIcc$ indicates a better model, balancing goodness of fit with model complexity. Both automatic methods selected the same best model($ARIMA(2,0,2)(0,1,1)$) with drift. They share the lowest AICc value suggesting the simpler stepwise search was effective and the more exhaustive search did not yield any better structure.
...
We can further check the significance of the estimated values using the `coef()` or `tidy()` function which presents all the models a tabular structure with their corresponding coefficient estimates, standard errors and p-values.
...
These validate that the model is a comprehensive representation of the cement production series, successfully accounting for the level, trend, seasonality and the residual autocorrelations
